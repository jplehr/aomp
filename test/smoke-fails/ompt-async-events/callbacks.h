...
static ompt_callback_buffer_request_t buffer_req_fn = 0;
static ompt_callback_buffer_complete_t buffer_comp_fn = 0;

// OMPT callbacks
static void buf_req_callback(int dev_num, ompt_buffer_t **buf, size_t *bytes) {
  // TODO: Something reasonable
}
static void buf_comp_callback(int dev_num, ompt_buffer_t *buf, size_t bytes, ompt_buffer_cursor_t begin, int buffer_owned) {
  // TODO: Something reasonable
}
...
ompt_start_trace_fn = (ompt_start_trace_t) lookup("ompt_start_trace");
  ...
  if (ompt_start_trace_fn) {
    printf("OMPT: Starting device-side tracing\n");
    ompt_start_trace_fn (NULL, buf_req_callback, buf_comp_callback);
  } else {
    printf("OMPT: Could not start device-side tracing\n");
  }
}
